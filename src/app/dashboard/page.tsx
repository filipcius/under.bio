...
export default async function DashboardPage({
  ...
  searchParams: Promise<{ black?: string; gift?: string }>;
}) {
      ...
      <section className="glass-card w-full p-6 sm:p-8 lg:p-10">
        ...
        {sp.black && (
          <div className="mb-6 rounded-xl border border-sky-300/30 bg-sky-400/10 px-4 py-3 text-sm text-sky-100">
            ...
            Lifetime {BLACK_NAME} unlocked — Style lab is yours forever.
          </div>
        )}
        {sp.gift && (
          <div className="mb-6 rounded-xl border border-sky-300/30 bg-sky-400/10 px-4 py-3 text-sm text-sky-100">
            Gift checkout started — their {BLACK_NAME} unlocks when Stripe confirms payment.
          </div>
        )}
            ...
            <div className="flex flex-wrap items-center justify-between gap-4">
              <div>
                ...
                <p className="mt-1 font-medium text-white">
                  Full style lab · ${BLACK_PRICE_USD} lifetime · pay once
                </p>
                <p className="mt-1.5 text-sm text-white/45">
                  Or invite 10 friends for 14 days free —{" "}
                  <Link
                    href="/dashboard/invites"
                    className="text-sky-200/85 underline-offset-2 hover:underline"
                  >
                    Invites
                  </Link>
                </p>
              ...
              <div className="w-full max-w-xs sm:w-56">
                <BlackCheckoutButton signedIn label={`Upgrade to ${BLACK_NAME}`} />
              </div>
            </div>
          </div>
        )}
